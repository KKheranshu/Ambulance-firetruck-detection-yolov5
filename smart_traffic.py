...
model = torch.hub.load(
    "ultralytics/yolov5", "custom", path="runs/train/emergency-detector-v5/weights/best.pt", force_reload=True
)
...
while True:
    ...
    for *xyxy, conf, cls in detections:
        ...
        cv2.putText(frame, f"{class_name}", (x1, y1 - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 2)
    ...
    for i in range(LANE_COUNT):
        ...
        cv2.putText(
            frame, f"Lane {i + 1}: {lane_counts[i]}", (x_start + 10, 40), cv2.FONT_HERSHEY_SIMPLEX, 0.6, color, 2
        )
    ...
    if green_lane is not None:
        cv2.putText(
            frame, f"🟢 GREEN: Lane {green_lane + 1}", (20, y_text), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 0), 2
        )
        y_text += 30
        if red_lanes:
            cv2.putText(
                frame,
                f"🔴 RED: Lane(s) {', '.join(red_lanes)}",
                (20, y_text),
        ...
        cv2.putText(frame, decision, (20, y_text), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 255), 2)
